 ...
 */
class Bookstore_controller extends MX_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('Bookstore_model', 'MBookstore');
    ...
    public function show_student()
    {
        ...
        $details_data = $this->MBookstore->get_all_studentdata($parent_id, $inst_id);
    ...
    public function show_bookstore_payment($admission_number, $inst_id)
    {
        $this->clearCache();
        ...
        $details_data = $this->MBookstore->get_student_profile_by_admission_number($admission_number, $inst_id);
        ...
        if (isset($details_data['data']) && !empty($details_data['data'])) {
            ...
            $pack_list = $this->MBookstore->studentpack_bill_select_oh($student_id);
    ...
    public function pack_details()
    {
        ...
        if ($this->input->is_ajax_request() == 1) {
            ...
            if (isset($student_details['data']) && !empty($student_details['data'])) {
                ...
                $details_data = $this->MBookstore->get_bill_pack_details($pack_id);

                $pack_list_data = $this->MBookstore->studentpack_bill_select_oh($student_id, $pack_id);
                ...
                echo json_encode(array('status' => 1, 'view' => $this->load->view('student/pack_details_view', $data, TRUE)));
                return TRUE;
            } else {
    ...
    public function place_order()
    {
        if ($this->input->is_ajax_request() == 1) {
            ...
            $online_deilery_data = $this->MBookstore->get_bookstore_online_order($pack_id);
            ...
            if ($payment_type == 1) {
                ...
                $data_prep['reference_no'] = 'CD' . date('dmyhis') . $pack_id;
                $status = $this->MBookstore->save_bookstore_online_order_delivery_details($data_prep);

                $student_personal_details = $this->MBookstore->get_ind_student_details($student_id);
                ...
                $subject = "Cash on Delivery Order Placed- Book Store Order No. " . $pack_data['barcode'];
                $mailto = $email;
                ...
                $email_message = "Cash on Delivery Order received for Book Store Order ,Details are as follows<br/><br/>";
                ...
                $subject = "Cash on Delivery order received for Book Store Order No. " . $pack_data['barcode'];
    ...
    { // function to send atom values
        ...
        $ru = base_url('bookstore/process-return-payment-data');
    ...
    public function process_payment()
    {
        ...
        if ($transactionResponse->validateResponse($_POST)) {
            ...
            $details_data = $this->MBookstore->get_bill_pack_details($pack_id);
            $pack_list_data = $this->MBookstore->studentpack_bill_select_oh($student_id, $student_order_data['pack_id']);
            $pack_data = $pack_list_data['data']['data'][0];
            ...
            if ($_POST['f_code'] == 'Ok' && !empty($pack_session_data[$pack_id])) {
                ...
                $data_prep['reference_no'] = 'OP' . date('dmyhis') . $student_order_data['pack_id'];
                ...
                if (!empty($status['data'])) {
                    ...
                    $status = $this->MBookstore->save_storecashbill($student_id, $student_order_data['pack_id'], $json_online_data, $json_online_item_data);
                    ...
                    $data_get['type'] = 1;
                    $email_message = $this->load->view('student/email_template_payment', $data_get, true);
                    $subject = "Payment Success for Book Store Order No. " . $pack_data['barcode'];
                    $mailto = $parent_email;
                    $mailcontent = $email_message;
                    //$cc = $this->get_cc_email($data['inst_id']);
                    $cc = '';
                    ...
                    $email_message = "Payment received for Book Store Order ,Details are as follows<br/><br/>";
                    $email_message .= "Order No : " . $pack_data['barcode'] . " <br/>";
                    $email_message .= "Payment Amount : " . $pack_data['final_total'] . " <br/>";
                    $email_message .= "Student Admission No : " . $student_data['Reg_No'] . " <br/>";
                    $email_message .= "Student Name : " . $student_data['Student_Name'] . " <br/>";

                    $subject = "Payment received for Book Store Order No. " . $pack_data['barcode'];
                    $mailto_sp = $this->get_support_email($inst_id);
                    $mailcontent = $email_message;
                    $email_res = send_smtp_mailer($subject, $mailto_sp, $mailcontent, $cc, $attachment_data);
                    $this->session->unset_userdata('pack_session_data');
                    redirect(base_url() . 'bookstore/payment-success');
                } else {
                    $data_get['type'] = 2;
                    $email_message = $this->load->view('student/email_template_payment', $data_get, true);
                    $subject = "Payment Failed for Book Store Order";
                    $mailto = $parent_email;
                    $mailcontent = $email_message;
                    $cc = '';
                    $email_res = send_smtp_mailer($subject, $mailto, $mailcontent, $cc);

                    $email_message = "Processing failed for the Payment received towards Book Store Order ,Details are as follows<br/><br/>";
                    $email_message .= "Order No : " . $pack_data['barcode'] . " <br/>";
                    $email_message .= "Payment Amount : Rs." . $pack_data['final_total'] . " <br/>";
                    $email_message .= "Student Admission No : " . $student_data['Reg_No'] . " <br/>";
                    $email_message .= "Student Name : " . $student_data['Student_Name'] . " <br/>";

                    $subject = "Processing failed -Payment received for Book Store Order No. " . $pack_data['barcode'];
                    $mailto_sp = SUPPORT_DEV_TEAM_EMAIL;
                    $mailcontent = $email_message;
                    $email_res = send_smtp_mailer($subject, $mailto_sp, $mailcontent, $cc);

                    $this->session->unset_userdata('pack_session_data');
                    redirect(base_url() . 'bookstore/payment-failed');
                }
            } else {
                ...
                $email_message = "Payment failed for Book Store Order ,Details are as follows<br/><br/>";
                $email_message .= "Order No : " . $pack_data['barcode'] . " <br/>";
                $email_message .= "Payment Amount : " . $pack_data['final_total'] . " <br/>";
                $email_message .= "Student Admission No : " . $student_data['Reg_No'] . " <br/>";
                $email_message .= "Student Name : " . $student_data['Student_Name'] . " <br/>";

                $subject = "Payment failed for Book Store Order No. " . $pack_data['barcode'];
                $mailto_sp = $this->get_support_email($inst_id);
                $mailcontent = $email_message;
                $email_res = send_smtp_mailer($subject, $mailto_sp, $mailcontent, $cc);
                $this->session->unset_userdata('pack_session_data');
    ...
    public function create_bill_print_link($billcode)
    {
        ...
        $bill_details = $this->MBookstore->get_bill_details_for_print($billcode);
    ...
    public function payment_ack_success()
    {
        ...
        $data['redirect_link'] = base_url() . 'payment-bookstore/' . $this->session->userdata('admn_no_enc') . '/' . $this->session->userdata('inst_id');
