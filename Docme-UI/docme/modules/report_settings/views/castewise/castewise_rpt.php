    ...
    <div class="row" style="margin-left: -29px !important; margin-right: -30px !important;">
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content" id="RptCastewise_loader">
                    ...
                    <div class="body">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group <?php
                                                        ...
                                                        ?>">
                                    <label>Caste</label><span class="mandatory"> *</span><br />

                                    <select name="caste_select" id="caste_select" class="form-control " style="width:100%;">

                                        <option selected value="-1">Select</option>
                                        <?php
                                        if (isset($caste_data) && !empty($caste_data)) {
                                            foreach ($caste_data as $caste) {
                                                if (!empty($caste['caste_name'])) {
                                                    echo '<option value ="' . $caste['caste_id'] . '">' . $caste['caste_name'] . '</option>';
                                                }
                                            }
                                        }
                                        ?>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <a href="javascript:void(0);" class="btn btn-primary btn-sm" onclick="submit_data('<?php echo $acdyr['Acd_ID']  ?>','<?php echo $class['Course_Det_ID']  ?>','<?php echo $caste['caste_id']  ?>');"> Report</a>
...
<script type='text/javascript'>
    ...
    $('#caste_select').select2({
        'theme': 'bootstrap'
    });


    function submit_data(Acd_ID, class_ID, caste) {
        $('#RptCastewise_loader').addClass('sk-loading');
        var Acd_ID = $('#acd_year').val();
        var class_ID = $('#class_select').val();
        var caste = $('#caste_select').val();

        if (Acd_ID == -1) {
            swal('', 'Academic Year is required.', 'info');
            $('#RptCastewise_loader').removeClass('sk-loading');
            return;
        }
        if (class_ID == -1) {
        ...
        if (caste == -1) {
            swal('', 'Caste is required.', 'info');
            $('#RptCastewise_loader').removeClass('sk-loading');
            return;
        }

        var ops_url = baseurl + 'report/stud-castewisepdf';
        $.ajax({
            type: "POST",
            cache: false,
            // async: false,
