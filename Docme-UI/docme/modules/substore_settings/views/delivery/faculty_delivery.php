                    ...
                    <div class="panel-body">
                        <div class=" input-group" style="padding-bottom: 5%;">
                            ...
                            <span class="input-group-btn">
                                <button type="button" id="button_id" class="btn btn btn-primary" onclick="search_faculty_voucher();"> <i class="fa fa-search"></i></button>
                                            ...
                                            <div class="ibox float-e-margins">
                                                <div class="ibox-title" style="padding: 5PX 15PX 2px 15px;min-height: 35px;">
                                                    ...
                                                    <?php if ($packlist['is_delivery_cosed'] == 1) { ?>
                                                        <a href="javascript:void(0)" onclick="select_items('<?php echo $packlist['id']; ?>');" <span class="label label-info pull-right" style="background-color: #0070A0;">Delivered </span>
                                                        </a>
                                                    <?php } else if ($packlist['is_pending'] == 1) { ?>
                                                        <a href="javascript:void(0)" onclick="select_items('<?php echo $packlist['id']; ?>');" <span class="label label-info pull-right">Pending </span>
                                                        </a>
                                                    <?php } else { ?>
                                                        <a href="javascript:void(0)" onclick="select_items('<?php echo $packlist['id']; ?>');" <span class="label label-info pull-right">For Delivery </span>
                                                        </a>
                                                    <?php } ?>
                                                </div>
                                                <div class="ibox-title" style="padding: 5PX 15PX 2px 15px;min-height: 35px;">
...
<script>
    ...
    function search_faculty_voucher() {
        $('#item-replace-container').html('');
        $('#item-data-container').html('');
        var searchvoucher = $("#searchitem").val();
        var emp_id = $("#emp_id").val();

        var ops_url = baseurl + 'sales/faculty-voucher-delivery';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
