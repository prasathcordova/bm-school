...
class Stock_model extends CI_Model {
    ...
    public function get_all_stockAllotmnt_list() {
        ...
        $stockAllot_data = transport_data_with_param_with_urlencode(array('action' => 'get_stock_allotment_list_substore',), $apikey);
    ...
    public function get_all_stockAllotmnt_list_out() {
        $apikey = $this->session->userdata('API-Key');
        $stockAllot_data = transport_data_with_param_with_urlencode(array('action' => 'get_stock_allotment_list_substore_out',), $apikey);
        if (is_array($stockAllot_data) && isset($stockAllot_data['status']) && $stockAllot_data['status'] == 1) {
            return $stockAllot_data['data'];
        } else {
            if (isset($stockAllot_data['message'])) {
...
//        dev_export($item_data);die;
    ...
    public function get_stock_data_for_store($storeid) {
        $apikey = $this->session->userdata('API-Key');
        $data_prep = array(
            'action' => 'get_current_stock_list_report_substore',
            'store_id' => $storeid
        );
...
//        dev_export($store_id);die;
        $substore_data = transport_data_with_param_with_urlencode(array('action' => 'save_Stock_allotment_sub_out', 'allotment_item_details' => $itemdata, 'store_id' => $store_id, 'total_qty' => $total_items, 'total_value' => $net_value, 'description' => $description), $apikey);
