...
class OH_packing_model extends CI_Model {
    ...
    public function remove_stud_data($template_config_id, $template_id, $formatted_student_data) {
        ...
        $acd_data = transport_data_with_param_with_urlencode(array(
            'action' => 'uniform_remove_oh_student_assign',
    ...
    public function uniform_get_student_openhouse($template_id, $openhouse_id) {
        $store_id = $this->session->userdata('store_id');;
        $apikey = $this->session->userdata('API-Key');
        $openhouse = transport_data_with_param_with_urlencode(array(
            'action' => 'uniform_get_student_openhouse',
            'template_id' => $template_id,
    ...
    public function save_stud_data($template_config_id, $template_id, $formatted_student_data) {
        $apikey = $this->session->userdata('API-Key');
        $acd_data = transport_data_with_param_with_urlencode(array(
            'action' => 'uniform_save_oh_student_assign',
            'template_config_id' => $template_config_id,
            'template_id' => $template_id,
            'student_data' => $formatted_student_data,
                ), $apikey);
...
//        return array($stream_id, $academic_year, $session_id, $class_id, $flag_status);
    ...
    public function get_student_data($admissionno, $stream_id, $academic_year, $session_id, $class_id, $batch_id, $gender, $religion) {
        $apikey = $this->session->userdata('API-Key');
        $data = transport_data_with_param_with_urlencode(array('action' => 'uniform_get_stud_for_ohassign', 'status' => 1, 'Acd_Year' => $academic_year, 'Stream_ID' => $stream_id,
...
//        dev_export($apikey);die;
        $oh_data = transport_data_with_param_with_urlencode(array('action' => 'uniform_select_item_oh_stud_assign', 'template_id' => $template_id, 'openhouse_id' => $openhouse_id), $apikey);
//        dev_export($oh_data);die;
    ...
    public function get_items_for_oh_stud_assign_for_item_assign_for_template($template_id) {
        $apikey = $this->session->userdata('API-Key');
//        dev_export($apikey);die;
        $oh_data = transport_data_with_param_with_urlencode(array('action' => 'uniform_select_item_oh_stud_assign', 'template_id' => $template_id), $apikey);
//        dev_export($oh_data);die;
        if (is_array($oh_data)) {
            return $oh_data['data'];
        } else {
    ...
    public function uniform_get_oh_stud_assign_data_search($openhouse) {
        $apikey = $this->session->userdata('API-Key');
        $oh_data = transport_data_with_param_with_urlencode(array('action' => 'uniform_get_oh_stud_assign_data','openhouse_name' =>$openhouse ), $apikey);
        if (is_array($oh_data)) {
            return $oh_data['data'];
        } else {
            return array(
    ...
    public function uniform_get_oh_stud_assign_data() {
        $apikey = $this->session->userdata('API-Key');
        $oh_data = transport_data_with_param_with_urlencode(array('action' => 'uniform_get_oh_stud_assign_data'), $apikey);
        if (is_array($oh_data)) {
            return $oh_data['data'];
        } else {
            return array(
    ...
    public function save_ohitem_assign($template_id, $total_qty, $sub_total, $vat, $discount, $final_item_string, $roundoff, $finaltotal, $discount_type) {
        $apikey = $this->session->userdata('API-Key');
        $oh_data = transport_data_with_param_with_urlencode(array('action' => 'uniform_save_oh_item_assign', 'template_id' => $template_id, 'total_qty' => $total_qty,
...
//        dev_export($store_id);die;
        $apikey = $this->session->userdata('API-Key');
        $oh_data = transport_data_with_param_with_urlencode(array('action' => 'uniform_get_stock_for_packing_substore', 'store_id' => $store_id), $apikey);
        if (is_array($oh_data)) {
            return $oh_data['data'];
        } else {
            return array(
...
//        dev_export($store_id);die;
        $apikey = $this->session->userdata('API-Key');
        $oh_data = transport_data_with_param_with_urlencode(array('action' => 'uniform_search_item_stock_for_substore', 'store_id' => $store_id, 'name' => $search, 'barcode' => $search, 'code' => $search), $apikey);
        if (is_array($oh_data)) {
            return $oh_data['data'];
        } else {
            return array(
