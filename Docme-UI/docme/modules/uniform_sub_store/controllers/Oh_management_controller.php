...
class Oh_management_controller extends MX_Controller {
    ...
    public function add_temp_openhouse() {
        ...
        if ($onload == 1) {
            ...
            $open_house_master = $this->Ohmodel->uniform_get_openhouse_master($id);
            ...
            $open_house_detail = $this->Ohmodel->uniform_get_openhouse_detail($id);
...
//            dev_export($open_house_detail);die;
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $status = $this->Ohmodel->uniform_edit_openhouse($master_id, $start_date, $end_date, $no_temp_st, $description, $formatted_template_id, $discount);
...
//            dev_export($open_house_detail);die;
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $status = $this->Ohmodel->uniform_edit_openhouse($master_id, $start_date, $end_date, $no_temp_st, $description, $formatted_template_id);
...
//        dev_export("hi");die;
        $onload = filter_input(INPUT_POST, 'load', FILTER_SANITIZE_NUMBER_INT);
        $id = filter_input(INPUT_POST, 'id', FILTER_SANITIZE_NUMBER_INT);
        $status = $this->Ohmodel->uniform_delete_openhouse($id);
...
//            dev_export('123');die;
    ...
    public function uniform_save_openhouse() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $status = $this->Ohmodel->uniform_save_openhouse($start_date, $end_date, $no_temp_st, $description, $formatted_template_id, $discount);
