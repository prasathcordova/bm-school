            ...
            <div class="panel-heading">
                ...
                <?php if ($details_data[0]['is_delivery_cosed'] != 0) { ?>
                    <span style="float: right;padding-top: 0px;">
                        <a href="javascript:void(0);" style="padding-top: 0px;" id="print_delivery" data-toggle="tooltip" title="Print of <?php echo $details_data[0]['delivery_number']; ?> " onclick="uniform_print_data('<?php echo $details_data[0]['id']; ?>')" class="btn btn-primary"> <i class="fa fa-print" style="color: white;font-size: 16px;"></i> </a>
                    </span>
                <?php } ?>
            </div>
            <div class="panel-body">
                <div class="title-action" style="text-align: right;padding-top: 0px; padding-bottom: 10px;">
                    ...
                    <?php if ($details_data[0]['is_delivery_cosed'] == 0) { ?>
                        ...
                        <?php } ?>
                        <a href="javascript:void(0);" id="save_delivery" onclick="uniform_submit_data('<?php echo $details_data[0]['id']; ?>', '<?php echo $details_data[0]['delivery_number']; ?>')" class="btn btn-primary"> Confirm Delivery </a>
                                            ...
                                            <?php if ($items['type'] == 1 || $items['type'] == 2) { ?>
                                                <?php if ($items['delivery_qty'] != $items['quantity']) { ?>
                                                    ...
                                                    <?php if ($items['delivery_qty'] == 0) { ?>
                                                        <a href="javascript:void(0)" onclick="uniform_replace_item('<?php echo $items['del_detail_id'] ?>', '<?php echo $items['item_id'] ?>', '<?php echo $price ?>', '<?php echo $items['type_id'] ?>', '<?php echo $items['quantity'] ?>', '<?php echo $items['pending_qty']; ?>');" data-toggle="tooltip" title="Replace Item"><i class="fa fa-exchange"></i></a>
                                                    <?php } ?>
...
<script>
    ...
    function uniform_replace_item(del_detail_id, item_id, price, type_id, qty, pending_qty) {
        if (pending_qty == 0) {
            var replace_qty = qty;
        } else {
        ...
        var ops_url = baseurl + 'uniform/substore/delivery-item-replace';
    ...
    function uniform_submit_data(delivery_id, delivery_number) {
        ...
        var ops_url = baseurl + 'uniform/delivery/delivery-save/';
        swal({
            title: "Are you sure?",
                ...
                $.ajax({
                    ...
                    success: function(result_s) {
                        ...
                        if (data.status == 1) {
                            ...
                            if (emp_id != 0) {
                                uniform_delivery_faculty(emp_id);
                                uniform_select_items(delivery_id)
                            }
    ...
    function uniform_print_data(delivery_id) {
        var ops_url = baseurl + 'uniform/delivery/delivery-note-print/';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
    ...
    function uniform_replace_OH(template_config_id, template_id) {
        var pack_id = $('#pack_id').val();
        var delivery_id = $('#delivery_id').val();
