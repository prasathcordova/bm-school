                    ...
                    <div class="panel-body">
                        <div class=" input-group" style="padding-bottom: 5%;">
                            ...
                            <span class="input-group-btn">
                                <button type="button" class="btn btn btn-primary" id="button_id" onclick="uniform_search_item('<?php echo $student['student_id'] ?>');"> <i class="fa fa-search"></i></button>
                                            ...
                                            <div class="ibox float-e-margins">
                                                <div class="ibox-title" style="padding: 5PX 15PX 2px 15px;min-height: 35px;">
                                                    ...
                                                    <?php if ($packlist['is_delivery_cosed'] == 1) { ?>
                                                        <a href="javascript:void(0)" onclick="uniform_select_items_delivery('<?php echo $packlist['id']; ?>');" <span class="label label-info pull-right" style="background-color: #0070A0;">Delivered </span>
                                                        </a>
                                                    <?php } else if ($packlist['is_pending'] == 1) { ?>
                                                        <a href="javascript:void(0)" onclick="uniform_select_items_delivery('<?php echo $packlist['id']; ?>');" <span class="label label-info pull-right">Pending </span>
                                                        </a>
                                                    <?php } else { ?>
                                                        <a href="javascript:void(0)" onclick="uniform_select_items_delivery('<?php echo $packlist['id']; ?>');" <span class="label label-info pull-right">For Delivery </span>
                                                        </a>
                                                    <?php } ?>
                                                </div>
                                                <div class="ibox-title" style="padding: 5PX 15PX 2px 15px;min-height: 35px;">
...
<script>
    ...
    input.addEventListener("keyup", function(event) {
        ...
        if (event.keyCode === 13) {
            ...
            if ($("#flag_status").val() == 1) {
                ...
                uniform_select_items_delivery(packet_view_id);
            }
        }
    });

    function uniform_select_items_delivery_packid(pack_id) {
    ...
    function uniform_search_item(student_id) {
        ...
        var ops_url = baseurl + 'uniform/sales/searchstud-voucher-delivery';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
...
<?php if (isset($pack_id)) { ?>
    <script>
        uniform_select_items_delivery_packid(<?php echo $pack_id ?>);
    </script>
