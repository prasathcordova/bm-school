            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    ...
                    <div class="tabs-container">
                        ...
                        <?php
                        echo form_open('details/item-edit', array('id' => 'item_save', 'role' => 'form'));
                        ?>
                            ...
                            <div id="tab-1" class="tab-pane active">
                                <div class="panel-body">
                                    ...
                                    <fieldset class="form-horizontal">
                                        <input type="hidden" id="item_id" name="item_id" value="<?php echo set_value('item_id', isset($item_id) ? $item_id : ''); ?>">
                                        ...
                                        <div class="form-group"><label class="col-sm-2 control-label">Item Code:</label>
                                            <div class="col-sm-10"><input type="text" class="form-control" placeholder="Item Code" name="item_code" id="item_code" disabled style="background-color: #ffffff" value="<?php echo set_value('item_code', isset($item_code) ? $item_code : ''); ?>"></div>
                                        </div>
                                        <div class="form-group"><label class="col-sm-2 control-label">Description:</label>
                                            <div class="col-sm-10">
                                                <textarea class="form-control summernote" value="" id="item_description" name="item_description" value=""><?php echo $item_description; ?></textarea>
                                                ...
                                                <select class="form-control" name="publisher" id="publisher">
                                                    ...
                                                    if (isset($publisher) && !empty($publisher)) {
                                                        foreach ($publisher as $pubs) {
                                                            if (isset($pub_id) && !empty($pub_id) && $pub_id == $pubs['pub_id']) {
                                                                echo '<option value="' . $pubs['pub_id'] . '" selected >' . $pubs['pub_name'] . '(Code:' . $pubs['pub_code'] . ')</option>';
                                                            } else {
                                                ...
                                                <select class="form-control" name="itemtype" id="itemtype">
                                                    ...
                                                    if (isset($itemtype) && !empty($itemtype)) {
                                                        foreach ($itemtype as $itemtypedata) {
                                                            if (isset($type_id) && !empty($type_id) && $type_id == $itemtypedata['itemtype_id']) {
                                                                echo '<option value="' . $itemtypedata['itemtype_id'] . '" selected >' . $itemtypedata['itemtype_name'] . '(Code:' . $itemtypedata['itemtype_code'] . ')</option>';
                                                            } else {
                                                ...
                                                <select class="form-control" name="itemedition" id="itemedition">
                                                    ...
                                                    if (isset($item_edition) && !empty($item_edition)) {
                                                        foreach ($item_edition as $item_editiondata) {
                                                            if (isset($edition_id) && !empty($edition_id) && $edition_id == $item_editiondata['id']) {
                                                                echo '<option value="' . $item_editiondata['id'] . '" selected >' . $item_editiondata['edition_name'] . '</option>';
                                                            } else {
                                                ...
                                                <select class="form-control" name="stock_category" id="stock_category">
                                                    ...
                                                    if (isset($stock_category) && !empty($stock_category)) {
                                                        foreach ($stock_category as $category) {
                                                            if (isset($cat_id) && !empty($cat_id) && $cat_id == $category['cate_id']) {
                                        ...
                                        <table class="table table-stripped table-bordered">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type="text" class="form-control" readonly="" placeholder="10" id="barcodedata_data" name="barcodedata_data" value="<?php echo $barcode; ?>">
                                                    </td>
...
<script type="text/javascript">
    ...
    function edit_submit_data() {
        ...
        var ops_url = baseurl + 'details/item-edit/';
        var item_id = $('#item_id').val();
        var item_name = $('#item_name').val().toUpperCase();
        var item_code = $('#item_code').val();
        var item_description = $(".summernote").summernote("code");
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (data.status == 1) {
                    //                    $('#add_type').show();
                    swal('Success', 'Item, ' + item_name + ' updated successfully.', 'success');
                    $('#data-view').html('');
