<div class="row" id="">
    <div class="panel panel-info">
        <div class="panel-heading">
            <i class="fa fa-info-circle" style="padding-right:10px;"></i>Student Vehicle Allocation
        <!--<a href="javascript:void(0)" onclick="allotment_student_vehicle();" id="close_button" data-toggle="tooltip" title="Close" style="float: right; color: #B22222;"><i class="fa fa-close"></i></a>-->
        </div>
        <div class="panel-body">
            <h3 id="admission_div_h" style="color: #1c84c6;">Student Direct Search<hr></h3>
            ...
            <h3 id="advanced_search_h" style="color: #1c84c6;">Student Advanced Search<hr></h3>
...
<script type="text/javascript">
    ...
    function search_name() {
        ...
        if (searchname.length < 3) {
            swal('', 'Please enter atleast three character ', 'info');
            return false;
        }
        var ops_url = baseurl + 'transport/search-studentname-for-allotment';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
...
//        alert(searchname);
        ...
        var ops_url = baseurl + 'transport/show-advanced-search-for-student-allotment';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
            url: ops_url,
//            data: {"load": 1, "searchname": searchname},
            data: {"load": 1, "stream_id": stream_id, "batch_id": batch_id, "searchname": searchname, "class_id": class_id, "academic_year": academic_year},
            success: function (result) {
                var data = JSON.parse(result)
                if (data.status == 1) {
                    $('#student-data-container').html('');
...
//        var class_data = JSON.stringify(class_id);
        var ops_url = baseurl + 'transport/show-batch-for-student-filter-of-student-allotment/';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
