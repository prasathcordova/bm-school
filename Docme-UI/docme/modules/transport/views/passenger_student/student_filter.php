...
<div class="row" id="">
    <div class="panel panel-info">
        <div class="panel-heading">
            <i class="fa fa-info-circle" style="padding-right:10px;"></i>Student Allotment
            <a href="javascript:void(0)" onclick="show_student_filter();" id="close_button" data-toggle="tooltip" title="Close" style="float: right; color: #B22222;"><i class="fa fa-close"></i></a>
        </div>
...
<script type="text/javascript">
    ...
    function search_name() {
        ...
        var ops_url = baseurl + 'transport/search-student-for-trip-allotment';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
    ...
    function searchadvance_filtername() {
        ...
        var ops_url = baseurl + 'transport/advancesearch-student-for-trip-allotment';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
    ...
    function changed_class() {
        ...
        if (class_id == -1) {
            ...
            $('#batch_id').append("<option value='-1'  bitdata-data ='1'>Select</option>");
            $('#batch_id').trigger("change");
        } else {
            var ops_url = baseurl + 'transport/passenger-batch-for-student-filter/';
            $.ajax({
                type: "POST",
                cache: false,
                async: false,
