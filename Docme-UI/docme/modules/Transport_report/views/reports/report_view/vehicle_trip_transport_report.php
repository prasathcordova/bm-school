...
<body>
    ...
    <table class="table table-bordered" width="100%" cellpadding="5">
        ...
        <tbody>
            ...
            ?>
                    <tr>
                        <td colspan="4" style="font-weight:bold">Vehicle : <?php echo !empty($data['pick_vehicle_reg']) ? $data['pick_vehicle_reg'] : (!empty($data['drop_vehicle_reg']) ? $data['drop_vehicle_reg'] : '')  ?></td>
                    </tr>
                    <tr>
                    ...
                    foreach ($data['travel_type'] as $travel_type => $data_travel) {
                        ...
                        if (isset($data['travel_type'][$travel_type])) {
                            foreach ($data['travel_type'][$travel_type] as $stop_id => $data_pick) {
                                ...
                                $k = 1; ?>
                                <tr>
                                    <td colspan="4" style="font-style:italic">Stop : <?php echo isset($data_pick['pickup_name']) ? $data_pick['pickup_name'] : $data_pick['drop_pickupName']; ?></td>
                                </tr>
                                <?php if (isset($data_pick['student_data'])) { ?>
                                    ...
                                    ?>
                                        <tr>
                                            <?php if ($i == 1) { ?>
                                                <!-- <td rowspan=<?php echo $span_size['total_span'][$trip_id] ?>><?php echo !empty($data['pick_vehicle_reg']) ? $data['pick_vehicle_reg'] : (!empty($data['drop_vehicle_reg']) ? $data['drop_vehicle_reg'] : '')  ?></td>
