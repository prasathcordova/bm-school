...
//        echo '<pre>';print_r($controller_method);die;
    ...
    protected function _detect_api_key()
    {
        ...
        $action_issued = filter_input(INPUT_POST,'action',FILTER_SANITIZE_STRING)?filter_input(INPUT_POST,'action',FILTER_SANITIZE_STRING):filter_input(INPUT_GET,'action',FILTER_SANITIZE_STRING);
        // Find the key from server or arguments
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            // Made change to the key query so that to check if the key is disabled or not.
            if ( ! ($row = $this->rest->db->where(array($this->config->item('rest_key_column')=> $key,'isactive' => 1))->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }

            $this->rest->key = $row->{$this->config->item('rest_key_column')};
            if($row->is_static == 1 && $action_issued != 'login_user') {
                return FALSE;
            }           
