...
/**
 * Description of Pick_student_pickchange_controller
 *
 * @author Chandrajith
 */
class Pick_student_pickchange_controller extends MX_Controller {
     public function __construct() {
        parent::__construct();
        $this->load->model('Pick_student_pickchange_model', 'MSP');
    }
    ...
    public function get_triplink_pick_data($params = NULL) {     
        ...
        $dbparams[0] = $params['API_KEY'];
          if (isset($params['student_id']) && !empty($params['student_id'])) {
            ...
            return array('data_status' => 0, 'error_status' => 1, 'message' => 'Student  is required', 'data' => FALSE);
        }           
          if (isset($params['acdyr']) && !empty($params['acdyr'])) {
            $dbparams[2] = $params['acdyr'];
        } else {
