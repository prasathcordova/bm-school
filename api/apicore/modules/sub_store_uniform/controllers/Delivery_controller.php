...
class Delivery_controller extends MX_Controller
{
    ...
    public function save_uniform_online_order_delivery_details($params)
    {
        $dbparams = array();
        $dbparams[0] = $params['API_KEY'];
        ...
        $packdetails = $this->MSDelivery->save_uniform_online_order_delivery_details($dbparams);
        if (!empty($packdetails) && is_array($packdetails)) {
            return array('data_status' => 1, 'error_status' => 0, 'message' => 'Data Loaded', 'data' => $packdetails);
        } else {
            return array('data_status' => 0, 'error_status' => 0, 'message' => 'No data available', 'data' => FALSE);
    ...
    public function get_uniform_online_order($params)
    {
        $dbparams = array();
        $dbparams[0] = $params['API_KEY'];
        if (isset($params['pack_id']) && !empty($params['pack_id'])) {
        ...
        $packdetails = $this->MSDelivery->get_uniform_online_order($dbparams);
        if (!empty($packdetails) && is_array($packdetails)) {
            return array('data_status' => 1, 'error_status' => 0, 'message' => 'Data Loaded', 'data' => $packdetails);
        } else {
            return array('data_status' => 0, 'error_status' => 0, 'message' => 'No data available', 'data' => FALSE);
