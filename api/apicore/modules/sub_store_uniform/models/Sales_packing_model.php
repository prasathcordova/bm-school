...
class Sales_packing_model extends CI_Model {
    ...
    public function save_student_packing($dbparams) {
        $this->db->flush_cache();
        $data = $this->db->query("[docme_uniform_store].[sales_packing_save_student] ?,?,?,?,?,?,?,?", $dbparams)->result_array();        
        if(isset($data[0])) {
            return $data[0];
        } else {
            return FALSE;
    ...
    public function save_faculty_packing($dbparams) {
        $this->db->flush_cache();
        $data = $this->db->query("[docme_uniform_store].[sales_packing_save_employee] ?,?,?,?,?,?,?,?", $dbparams)->result_array();        
        if(isset($data[0])) {
            return $data[0];
        } else {
            return FALSE;
