...
select *,vdm.vehicle_id,em.Middle_name,em.Last_name  from docme_transport.tbl_vehicle_registration vreg 
left join docme_transport.tbl_vehicle_driver_map vdm on vdm.vehicle_id = vreg.id and vdm.isactive = 1
left join dbo.PAY_EMPLOYEE_MASTER em on em.Emp_id = vdm.emp_id and vdm.isactive = 1
where vreg.id = @EMP_ID AND  vreg.instId = @inst_id and vreg.isActive = 1
